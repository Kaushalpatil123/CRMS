...
const getService = async () => {
  setLoading(true);
  try {
    const response = await axios.get(`${server}/api/visualinternationalresume/plans`);
...
try {
  const response = await axios.put(`${server}/api/visualinternationalresume/update/${serviceKey}/${editId}`,requestData, {
    headers: {
      Accept: '*/*',
        ...
        <Grid item xs={12}>
          ...
          <List sx={{ width: 1, p: 0 }}>
            ...
            >
              ...
              <ListItemText
                primary={<Typography variant="h4" className='text-orange-600 w-[80%]'>Visual International Resume Services</Typography>}
...
export default VisualInternational;
