...
const ZapYourResume = () => {
  ...
  useEffect(() => {
    const getService = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${server}/api/zapyourresumeservice/plans`);
  ...
  async function handleSubmit(e){
   ...
   console.log('this is the ID of Plan', editId)
    ...
    try {
      const response = await axios.put(`${server}/api/zapyourresumeservice/update/${editId}`,requestData, {
        headers: {
          Accept: '*/*',
          ...
          <Grid item xs={12}>
            <List sx={{ width: 1, p: 0 }}>
              ...
              >
                ...
                <ListItemText
                  primary={<Typography variant="h4" className='w-[80%] text-orange-600'>{service?.serviceName}</Typography>}
