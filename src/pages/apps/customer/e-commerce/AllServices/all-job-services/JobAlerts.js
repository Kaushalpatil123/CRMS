...
const JobAlerts = () => {
  ...
  useEffect(() => {
    const getService = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${server}/api/JobAlertonmailmessage/job-alerts`);
        if (response.data.status !== "success") {
          throw new Error();
        }
        setService(response.data?.jobAlerts[0]);
        console.log(response.data?.jobAlerts[0]);
      } catch (error) {
  ...
  async function handleSubmit(e){
   ...
   console.log('this is the ID of Plan', editId)
    ...
    try {
      const response = await axios.put(`${server}/api/JobAlertonmailmessage/job-alerts/${editId}`,requestData, {
        headers: {
          Accept: '*/*',
