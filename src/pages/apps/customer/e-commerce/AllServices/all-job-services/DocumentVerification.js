...
const DocumentVerification = () => {
  ...
  useEffect(() => {
    const getService = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${server}/api/documentverificationservice/plans`);
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await axios.put(`${server}/api/documentverificationservice/update/${editId}`, requestData, {
        headers: {
          Accept: '*/*',
    ...
    // Close the dialog box
        setEditPrice(''); // Clear the form fields
        setEditPlanName('');
  }
