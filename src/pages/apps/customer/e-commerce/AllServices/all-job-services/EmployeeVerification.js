...
const EmployeeVerification = () => {
  ...
  useEffect(() => {
    const getService = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${server}/api/empbgvarification/plans`);
  ...
  async function handleSubmit(e){
   ...
   console.log('this is the ID of Plan', editId)
    ...
    try {
      const response = await axios.put(`${server}/api/empbgvarification/update/${editId}`,requestData, {
        headers: {
          Accept: '*/*',
          ...
          <Grid item xs={12}>
            <List sx={{ width: 1, p: 0 }}>
              ...
              >
                ...
                <ListItemText
                  primary={<Typography variant="h4" className='text-orange-600'>{service?.empbgserviceName}</Typography>}
