...
const CoachBooking_details = () => {
  const [BookingData, setBookingData] = useState('');

  const { id } = useParams();

  useEffect(() => {
    const fetchBookingCoach = async () => {
      const token = localStorage.getItem('token');
      if (!token) {
        // navigate("/");
      } else {
        try {
          const response = await axios.get(`${server}/api/booking/coach-bookings/${id}`, {
            headers: {
...
export default CoachBooking_details;
