...
const EditAddress = ({ open, handleClose, handleAddressUpdate, address }) => {
  ...
  useEffect(() => {
    if (selectedAddress && Array.isArray(address)) {
      const selected = address.find((addr) => addr.address1 === selectedAddress);
      if (selected) {
        setAddressState({
  ...
  const handleSave = () => {
    const combinedAddress = `${addressState.address1}, ${addressState.address2 ? `${addressState.address2}, ` : ''}${addressState.city}, ${
      addressState.state
    }, ${addressState.country} - ${addressState.pincode}`;

    handleAddressUpdate({ ...addressState });
