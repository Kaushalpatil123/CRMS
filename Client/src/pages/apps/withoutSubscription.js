          ...
          <TableBody>
            {paginatedData?.map((row) => (
              ...
              <TableRow key={row._id}>
                ...
                <TableCell style={{ width: '25%' }}>
                  <Stack direction="row" spacing={1.5} alignItems="center">
                    <Typography> {new Date(row?.subscription?.purchaseDate).toLocaleString() || 'NA'}</Typography>
                  </Stack>
                </TableCell>{' '}
                <TableCell style={{ width: '25%' }}>
                  <Stack direction="row" spacing={1.5} alignItems="center">
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      if (!token) {
        ...
        try {
          const response = await axios.get(`${server}/api/school/user/without-subscription`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
