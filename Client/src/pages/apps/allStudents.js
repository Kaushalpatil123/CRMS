            ...
            {paginatedData?.map((row) => (
              ...
              <TableRow key={row._id}>
                ...
                <TableCell style={{ width: '25%' }}>
                  <Stack className="flex flex-col justify-start align-middle text-left">
                ...
                <TableCell style={{ width: '25%' }}>
                  <Stack direction="row" spacing={1.5} alignItems="center">
                    <Typography> {row?.DOB || 'NA'}</Typography>
                  </Stack>
                </TableCell>
                <TableCell style={{ width: '25%' }}>
                  <Stack direction="row" spacing={1.5} alignItems="center">
                  ...
                  <Stack direction="row" spacing={1.5} alignItems="center">
                    <Typography>
                      {row?.isDeleted === true ? (
                        ...
                        <div className="px-4 py-1 text-[14px] text-red-800 bg-red-100 rounded-full ">False</div>
                      )}
                    </Typography>
                  </Stack>
                </TableCell>
                ...
                {/* <TableCell style={{ width: '25%' }}>
                  <Stack direction="row" spacing={1.5} alignItems="center">
                    <Typography> {row?.subscription?.expirationDate || 'NA'}</Typography>
                  </Stack>
                </TableCell> */}
                {/* <TableCell style={{ width: '25%' }}>
                  <Stack direction="row" spacing={1.5} alignItems="center">
                    <Typography> {row?.subscription?.Status == true ? 'TRUE' : 'FALSE' || 'NA'}</Typography>
                  </Stack>
                </TableCell> */}
                {/* <TableCell style={{ width: '25%' }}>
...
const AllStudents = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      if (!token) {
        ...
        try {
          const response = await axios.get(`${server}/api/school/student`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
  ...
  const columns = useMemo(
    () => [
      ...
      { Header: 'isSuspended', accessor: 'isSuspended' },
