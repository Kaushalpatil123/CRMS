...
const JobSearchAssistant = () => {
  ...
  useEffect(() => {
    const getService = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${server}/api/jobService/job-services`);
        if (response.data.status !== "success") {
          throw new Error();
        }
        setService(response.data?.jobServices[0]);
        console.log(response.data?.jobServices[0]);
      } catch (error) {
  ...
  async function handleSubmit(e) {
    ...
    // Create an updatedPlans array with the updated price
    const updatedPlans = service?.plans.map(plan => {
      if (plan.durationMonths === editPlanName) {
        return { durationMonths: plan.durationMonths, price: editPrice };
      } else {
        return { durationMonths: plan.durationMonths, price: plan.price };
      }
    });
  
    // Construct the requestData object with jobServiceName and updatedPlans array
    const requestData = {
      jobServiceName: service?.jobServiceName,
      plans: updatedPlans
    };
  
    try {
      const response = await axios.put(`${server}/api/jobService/update-plans/${editId}`, requestData, {
        headers: {
          Accept: '*/*',
          ...
          <Grid item xs={12}>
            <List sx={{ width: 1, p: 0 }}>
              ...
              >
                ...
                <ListItemText
                  primary={<Typography variant="h4" className='text-orange-600 w-[70%]'>{service?.jobServiceName}</Typography>}
