...
const InterviewPrepration = () => {
  ...
  useEffect(() => {
    const getService = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${server}/api/interviewpreparationservice/plans`);
  ...
  async function handleSubmit(e){
   ...
   console.log('this is the ID of Plan', editId)
    ...
    try {
      const response = await axios.put(`${server}/api/interviewpreparationservice/update/${editId}`,requestData, {
        headers: {
          Accept: '*/*',
