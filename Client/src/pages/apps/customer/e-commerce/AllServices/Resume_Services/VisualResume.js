...
const getService = async () => {
  setLoading(true);
  try {
    const response = await axios.get(`${server}/api/visualresumeservice/plans`);
...
try {
  const response = await axios.put(`${server}/api/visualresumeservice/update/${serviceKey}/${editId}`,requestData, {
    headers: {
      Accept: '*/*',
        ...
        <Grid item xs={12}>
          ...
          <List sx={{ width: 1, p: 0 }}>
            ...
            >
              ...
              <ListItemText
                primary={<Typography variant="h4" className='text-orange-600'>Visual Resume Services</Typography>}
