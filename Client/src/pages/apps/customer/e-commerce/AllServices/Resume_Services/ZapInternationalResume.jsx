...
const ZapInternationalResume = () => {
  ...
  useEffect(() => {
    const getService = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${server}/api/zapresumeinternationalservice/plans`);
        if (response.data.status !== "success") {
          throw new Error();
        }
        setService(response.data?.price[0]);
        console.log(response.data?.price[0]);
  ...
  async function handleSubmit(e){
   ...
   console.log('this is the ID of Plan', editId)
    ...
    try {
      const response = await axios.put(`${server}/api/zapresumeinternationalservice/update/${editId}`,requestData, {
        headers: {
          Accept: '*/*',
          ...
          <Grid item xs={12}>
            <List sx={{ width: 1, p: 0 }}>
              ...
              >
                ...
                <ListItemText
                  primary={<Typography variant="h4" className="text-orange-600 w-[70%]">{service?.serviceName}</Typography>}
                //   secondary={<Typography color="text.secondary">{customer.role}</Typography>}
                />
              </ListItem>
              <ListItemText
                  primary={<Typography variant="h4">INR { service?.price}/-</Typography>}
                //   secondary={<Typography color="text.secondary">{customer.role}</Typography>}
                />
  ...
  <Dialog
      ...
      >
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                          <Stack spacing={1.25}>
                        <InputLabel htmlFor="customer-name">{editPlanName}</InputLabel>
                        <TextField
                          fullWidth
                          id="customer-name"
...
export default ZapInternationalResume;
