...
const ResumeCritique = () => {
  ...
  useEffect(() => {
    const getService = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${server}/api/resumecritiqueservice/plans`);
  ...
  async function handleSubmit(e){
   ...
   console.log('this is the ID of Plan', editId)
    ...
    try {
      const response = await axios.put(`${server}/api/resumecritiqueservice/update/${editId}`,requestData, {
        headers: {
          Accept: '*/*',
