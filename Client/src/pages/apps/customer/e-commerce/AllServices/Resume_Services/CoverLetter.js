...
const CoverLetter = () => {
  ...
  useEffect(() => {
    const getService = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${server}/api/coverletterservice/plans`);
  ...
  async function handleSubmit(e){
   ...
   console.log('this is the ID of Plan', editId)
    ...
    try {
      const response = await axios.put(`${server}/api/coverletterservice/update/${editId}`,requestData, {
        headers: {
          Accept: '*/*',
