...
function ReactTable({ columns, data, renderRowSubComponent, setgroundData, setUserData, row }) {
  ...
  const handleEdit = (details) => {
    ...
    setEditschoolgroundName(details?.name);
                ...
                >
                  <TableCell className="text-center" style={{ width: '25%' }}>
                    <Stack direction="row" spacing={1.5} alignItems="center" justifyContent='center'>
                      ...
                      <Typography className='text-[12px]'>{row?.groundId?.school.institutionId || 'NA'}</Typography>
                    </Stack>
                  </TableCell>
                  ...
                  <TableCell className='text-center' style={{ width: '25%' }}>
                    <Stack direction="row" alignItems="center" className="flex justify-center text-center gap-1 ">
                      {row?.groundId?.sport_name?.map((r) => (
                        <Typography className="border-[2px] border-[#5f6368]/10 text-[14px] bg-gray-50 text-gray-600 rounded-full px-2 py-[2px] ">
                          {r}
                        </Typography>
                      )) || 'NA'}
                    </Stack>
                      ...
                      {/* <Typography>
                        {row?.date && row?.endTime
                          ? (() => {
                              ...
                              const [hours, minutes] = row.endTime.split(':').map((part) => parseInt(part, 10));
                              const endTimeString = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
                              const dateTimeString = `${formattedDate}T${endTimeString}`;
                              console.log('Check dateTimeString:', dateTimeString);
                              const dateTime = new Date(dateTimeString);
                              console.log('Check dateTime:', dateTime);
                  ...
                  <TableCell style={{ width: '25%' }}>
                    <Stack direction="row" alignItems="left" justifyContent="left" spacing={0}>
                      ...
                      <Tooltip
                        ...
                        title="View"
                      >
                        <IconButton color="success" onClick={() => navigate(`/apps/ground-booking-details/${row._id}`)}>
                          <Eye />
                        </IconButton>
                      </Tooltip>
                                        ...
                                        <List sx={{ py: 0 }}>
                                          ...
                                          <ListItem divider>
                                            <Grid container spacing={matchDownMD ? 0.5 : 3}>
                                              ...
                                              <Grid item xs={12} md={6}>
                                                <Stack spacing={0.5}>
                                                  <Typography color="secondary">{schoolData?.sportsgrounds || 'NA'}</Typography>
                                                </Stack>
                                              </Grid>
                                            </Grid>
                                          </ListItem>
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      if (!token) {
        ...
        try {
          const response = await axios.get(`${server}/api/booking/ground-bookings`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
